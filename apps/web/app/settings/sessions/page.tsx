...
import { apiGet, getAccessToken } from '../../../lib/client-api';
...
export default function SessionsPage(): React.ReactElement {
  ...
  async function loadSessions(): Promise<void> {
    ...
    if (!token) {
      setMessage('Please sign in again to view active sessions.');
      return;
    }
  ...
  async function revoke(id: string): Promise<void> {
    const token = getAccessToken();
    if (!token) {
      setMessage('Please sign in again to revoke sessions.');
      return;
    }
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL ?? 'http://localhost:4000/api'}/auth/sessions/${id}`,
      {
        method: 'DELETE',
