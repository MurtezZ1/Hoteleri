    ...
    <AppShell>
      <section className="mb-6 rounded-md border border-slate-200 bg-white p-5 shadow-[0_10px_28px_rgba(15,31,61,0.06)]">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-sm font-medium text-slate-500">Wednesday, July 22, 2026</p>
            <h2 className="mt-1 text-2xl font-semibold tracking-normal text-navy">Operations overview</h2>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">
              Track arrivals, room readiness, payments, and channel performance from one focused workspace.
            </p>
          </div>
          <DashboardActions />
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 2xl:grid-cols-6">
        {metrics.map((metric, index) => (
          <MetricCard
            key={metric.label}
            metric={metric}
            tone={index === 3 ? 'green' : index === 5 ? 'amber' : index === 2 ? 'slate' : 'blue'}
          />
        ))}
      </div>
      ...
      <div className="mt-6 grid min-w-0 gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
        <div className="min-w-0">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="text-lg font-semibold text-navy">Reservation calendar</h2>
              <p className="text-sm text-slate-500">Weekly room plan with payment and arrival status.</p>
            </div>
            <CalendarViewToggle />
          </div>
          <CalendarBoard />
        ...
        <aside className="space-y-4">
          ...
          <section className="rounded-md border border-slate-200 bg-white p-5 shadow-[0_10px_28px_rgba(15,31,61,0.06)]">
            <h2 className="text-lg font-semibold text-navy">Upcoming arrivals</h2>
            <div className="mt-4 space-y-3 text-sm">
              {['Elena Novak - Room 101', 'Jon Bell - Suite 201', 'Amina Rossi - Room 102'].map((item) => (
                <div key={item} className="rounded-md border border-slate-100 bg-slate-50 px-3 py-2 font-medium text-slate-700">
                  {item}
                </div>
              ))}
            </div>
          ...
          <section className="rounded-md border border-slate-200 bg-white p-5 shadow-[0_10px_28px_rgba(15,31,61,0.06)]">
            ...
            <div className="mt-4 space-y-3">
              {activity.map(([time, label, meta]) => (
                <div key={`${time}-${label}`} className="grid grid-cols-[48px_1fr] gap-3 text-sm">
